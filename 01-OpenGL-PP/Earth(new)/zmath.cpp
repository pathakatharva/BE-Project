...
void myInfiniteProj(float InfiniteMatrix[], float fovy, float aspect, float n, float f)
{
	//function prototypes
	float radians(float);
	...
	for (i = 0; i < 16; i++)
	{
		if (i == 0)
			...
			InfiniteMatrix[i] = -1.0f;
		else if (i == 11)
			InfiniteMatrix[i] = -1.0f;
		else if (i == 14)
			InfiniteMatrix[i] = C;
		else
			InfiniteMatrix[i] = 0.0f;
	}
}
...
//to be used in the case -> matrix1 = matrix1 * matrix2 (eg: modelViewMatrix = modelViewMatrix * rotationMatrix)
...
void createViewMatrix(float viewMatrix[],Camera cam)
{
	...
	zmath::vec3 cameraPos = cam.getPosition();
