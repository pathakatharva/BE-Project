...
void multiplyMatrices1(float matrix1[], float matrix2[])
{
	...
	for (i = 0; i < 16; i++)
	{
		...
		{
			product = product + (matrix2[j] * bufferMatrix[k]);
		}
		matrix1[i] = product;
...
void createTransformationMatrix(vec3 translate, vec3 scale, float  transformMatrix[], vec3 rotate)
{
	float tempMatrix[16];

	myIdentity(transformMatrix);
	myIdentity(tempMatrix);



	myTranslate(transformMatrix, translate[0], translate[1], translate[2]);

	myScale(tempMatrix, scale[0], scale[1], scale[2]);
	multiplyMatrices1(transformMatrix, tempMatrix);

	myRotate(tempMatrix, rotate[0], 1.0f, 0.0f, 0.0f);
	multiplyMatrices1(transformMatrix, tempMatrix);

	myRotate(tempMatrix, rotate[1], 0.0f, 1.0f, 0.0f);
	multiplyMatrices1(transformMatrix, tempMatrix);

	myRotate(tempMatrix, rotate[2], 0.0f, 0.0f, 1.0f);
	multiplyMatrices1(transformMatrix, tempMatrix);
...
//	FILE* viewXfileLog = NULL;
//
//	myIdentity(rotationMatrix);
//	myIdentity(translateMatrix);
//	myIdentity(viewMatrix);
//	
//	pmath::vec3 cameraPos = cam.getPosition();
//	myRotate(rotationMatrix,cam.getPitch(), 1.0f, 0.0f, 0.0f);
//	multiplyMatrices1(viewMatrix, rotationMatrix);
//
//	myRotate(rotationMatrix, cam.getYaw(), 0.0f, 1.0f, 0.0f);
//	multiplyMatrices1(viewMatrix, rotationMatrix);
//
//	
//	myTranslate(translateMatrix, -cameraPos[0], -cameraPos[1], -cameraPos[2]);
//	multiplyMatrices1(viewMatrix, translateMatrix);
//
//	//myRotate(rotationMatrix, cam.getRoll(), 0.0f, 0.0f, 1.0f);
//	//multiplyMatrices1(viewMatrix, rotationMatrix);
//
//	
//	/*if (fopen_s(&viewXfileLog, "Log/viewMatrxLog.txt", "w") != 0)
//	{
//		MessageBox(NULL, TEXT("viewMatrxLog File Can Not Be Created\nExitting ..."), TEXT("Error"), MB_OK | MB_TOPMOST | MB_ICONSTOP);
//		exit(0);
//	}
//	else
//	{
//		fprintf(viewXfileLog, "viewMatrxLog File Is Successfully Opened.\n");
//	}
//
//	fprintf(viewXfileLog, "viewMatrix==");
//	for (int i = 0; i < 16; i+=4)
//	{
//		fprintf(viewXfileLog, "\n%f %f %f %f", viewMatrix[i], viewMatrix[i+1], viewMatrix[i+2], viewMatrix[i+3]);
//	}
//	if (viewXfileLog)
//	{
//		fclose(viewXfileLog);
